...
error(nargchk(4, 4, nargin));%check the num of parameters

[fid, message] = fopen(fn, 'w');
if fid < 0, error(['Cannot open the file ' fn '\n' message]); end

nver = numel(shape)/3;%num of vertices equals num of shape/3
nface = numel(tl)/3;%num of faces equals num of tl/3
...
fprintf(fid, 'element vertex %d\n', nver);%num of vertices
fprintf(fid, 'property float %s\n', 'x', 'y', 'z');%first write coordinate data x,y,z
fprintf(fid, 'property uchar %s\n', 'red', 'green', 'blue');%then write tex data r,g,b
...
tl = tl-1;
new_tl = zeros(3, size(tl,1), 'int32');%create a 3*tl matrix of zeros
new_tl(1,:) = tl(:,2)';%exchange data row1 with row2
new_tl(2,:) = tl(:,1)';%
new_tl(3,:) = tl(:,3)';
nver_per_face = uint8(3);
for i=1:nface
%   fwrite(fid, nver_per_face, 'uchar');%all faces are triangle
  fprintf(fid,'%d ',nver_per_face);
  fprintf(fid,'%d %d %d\n',new_tl(1,i),new_tl(2,i),new_tl(3,i));
