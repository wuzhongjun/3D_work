...
new_num = 10;
for i = 1 : new_num
    img_str{i} = strcat(num2str(i,'%03d'),'_01');%to add more frontal face,change this cell
end
%   img_str = {'peach2','wu'};
...
%          exe_str{i} = strcat('D:\learning_opengl\stasm4.1.0\vc10\minimal.exe',frontal_face_dir,img_str{i},'.jpg',xy_dir,num2str(i),'.txt'); 
% %          exe_str = exe_str{1};
%          system(exe_str{i});
%     end
%          tmp_str = ['D:\learning_opengl\BaselFace\02_xy_ordinate\' num2str(i) '.txt'];
...
%% build GEM  
    ...
    vertex_xy = vertex(:,1:2);
     for i = 1:size(vertex_xy,1)
       ...
       save('D:\learning_opengl\BaselFace\02_save_mat\gem_vertex_v1.mat','vertex','tex_vertex','vertex_dtdt');
...
% end
%  for i = 1:new_num
%     ri_dir{i} = strcat('D:\learning_opengl\BaselFace\02_frontal_face\',img_str{i},'.jpg');
% %     img_dir = img_dir{i};
% %     i = 1;
%     inp{i} = imread(ri_dir{i});
%     inp_width{i} = size(inp{i},1);
%     inp_height{i} = size(inp{i},2);
%     tim = toc;
...
%     ge_tex{i} = zeros(size(new_xy{i},1),3);
...
%         ge_tex{i}(j,1) = inp{i}( inp_height{i} + 1 - (int32( new_xy{i}(j,2) ) + center_y{i}) , int32( new_xy{i}(j,1) ) + center_x{i} , 1); 
...
% %%
% %     h = figure(3);
% %     my_display_face_2d(vertex,tex_vertex,vertex_dtdt,rp);
%     h = figure(2);
