function [ out_image] = five_points_to_frontal_FERET( in_image_name, five_xy )
...
%     global vertex_dtdt;
    ...
    if out_image_name(6:7) == 'ba'
        P_angle = 1.1;
    elseif out_image_name(6:7) == 'bb'
        P_angle = 38.9;
    elseif out_image_name(6:7) == 'bc'
        P_angle = 27.4;
    elseif out_image_name(6:7) == 'bd'
        P_angle = 18.9;
    elseif out_image_name(6:7) == 'be'
        P_angle = 11.2;
    elseif out_image_name(6:7) == 'bf'
        P_angle = -7.1;
    elseif out_image_name(6:7) == 'bg'
        P_angle = -16.3;
    elseif out_image_name(6:7) == 'bh'
        P_angle = -26.5;
    elseif out_image_name(6:7) == 'bi'
        P_angle = -37.9;
    end
    P_angle = P_angle - 1.1;
...
%         five_gem{cnt}(5, 3) = five_gem{cnt}(4, 3);        
...
%         plywrite(fullfile(outDir4,tmptmp_str{i}), tmp_ge, repmat(new_gem_tex,3,1), vertex_dtdt); 
    ...
    if original == 0
        cur_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_profile_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal.bmp'];
    else
        cur_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_original_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal.bmp'];
    end
...
%         if abs(P_angle_y{cnt}) > 10 
    ...
    if original == 0   
        if half == 0
            ...
            cur_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_half_profile_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal.bmp'];            
        end
    else
        if half == 0
            cur_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_original_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal.bmp'];
        else
            cur_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_half_original_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal_1.bmp'];
            cur_img1 = cur_img( :, 1 : (window_width - 1)/2);
            imwrite( cur_img1, cur_img_dir);
            cur_img = cur_img( :, (window_width - 1)/2 + 2 : window_width);
            cur_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_half_original_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal_2.bmp'];
        end
    end
        imwrite(cur_img,cur_img_dir);
        cnt = 2;
        resize_img = imresize(cur_img,0.2,'bilinear');
    if original == 0
        if half == 0
            resize_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_profile_frontal_resize\' out_image_name(1:end-4) '_' num2str(cnt) '_resize.bmp'];
        else
            resize_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_half_profile_frontal_resize\' out_image_name(1:end-4) '_' num2str(cnt) '_resize.bmp'];
        end    
    else
        if half == 0
            resize_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_original_frontal_resize\' out_image_name(1:end-4) '_' num2str(cnt) '_resize.bmp'];
        else
            resize_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_half_original_frontal_resize\' out_image_name(1:end-4) '_' num2str(cnt) '_resize_1.bmp'];
            resize_img1 = imresize(cur_img1,0.2,'bilinear');
            imwrite( resize_img1, resize_img_dir);
            resize_img_dir = ['D:\learning_opengl\BaselFace\02_FERET_half_original_frontal_resize\' out_image_name(1:end-4) '_' num2str(cnt) '_resize_2.bmp'];            
        end
    end
