function my_display_face_2d (shp, tex, tl, rp)
	colormap gray
	shp = reshape(shp, [ 3 prod(size(shp))/3 ])'; %R n*3
% 	tex = reshape(tex, [ 3 prod(size(tex))/3 ])'; %R n*3 rgb
    tex = reshape(tex, [ 1 prod(size(tex)) ])'; %R n*1 gray
	tex = min(tex, 255);
...
% 	);
   ...
   mesh_h = trimesh(...
		tl, shp(:, 1), shp(:,2),  shp(:, 3),'EdgeColor', 'none','FaceVertexCData', tex/255,'FaceColor', 'interp','FaceLighting', 'phong');
axis([-100000 100000 -100000 100000 -100000 200000]);
	set(gca, ...
