...
acc = zeros(1,7);
acc_wrong = zeros(1,7);
% whole_ans_v2 = zeros(3,int32(size(img_dir,1)));
load('whole_ans_v2.mat','whole_ans_v2');
num = 1;
if num > 213
    num = num - 1;
end
i = 1;
% a = find(whole_ans_v2(1,[i:7:1743])~=whole_ans_v2(2,[i:7:1743]));
...
% for i = (a.*7-5)
    ...
    [dist_index,P_angle] = face_recognition_v2(strcat('D:\learning_opengl\MPIE\croppedimg\',img_dir(i).name),img_str ,std_num,i);
%     pause; 

    whole_ans_v2(1,i) = std_num;    
    whole_ans_v2(2,i) = dist_index;
    whole_ans_v2(3,i) = int32(P_angle);
    save('whole_ans_v2.mat','whole_ans_v2');
    pause;
end
...
rec_rate2 = zeros(1,7);
for i = 1:7
    rec_rate(i) = size(find(whole_ans_v2(1,[i:7:1743])==whole_ans_v2(2,[i:7:1743])),2)/249;
end
