function [ angle] = five_points_to_frontal_LFW( in_image_name, five_xy, fid )
...
%     global vertex_dtdt;
    ...
    if disp_img == 1
        ...
        imshow(in_image);
        hold on;
        plot(five_xy(1:2:end),five_xy(2:2:end),'+r');  hold on;
    end
...
%         five_gem{cnt}(4, 1) = -five_gem{cnt}(5, 1);
%         five_gem{cnt}(4, 2) = five_gem{cnt}(5, 2);
%         five_gem{cnt}(4, 3) = five_gem{cnt}(5, 3);
%         five_gem{cnt}(3, 1) = 0;
        all_gem{cnt} = tmp_ge';
        ...
        if disp_img == 1
            figure(1);
            plot(re_2d{cnt}(1:2:end),re_2d{cnt}(2:2:end),'.b');
            hold off;
        end
...
%     fprintf(fid,'\nname:%s p_angle_y:%f p_angle_x:%f p_angle_z:%f\n',out_image_name,P_angle_y{cnt},P_angle_x{cnt},P_angle_z{cnt});
    angle = [P_angle_y{cnt} P_angle_x{cnt} P_angle_z{cnt}];
...
%             if i ~= [5034,7644,12500,5159,3863]
                if all_gem_2d{cnt}(2*i) < 1
                    all_gem_2d{cnt}(2*i) = 1;
                end
                if all_gem_2d{cnt}((i - 1)*2+1) < 1
                    all_gem_2d{cnt}((i - 1)*2+1) = 1;
                end
...
%         plywrite(fullfile(outDir4,tmptmp_str{i}), tmp_ge, repmat(new_gem_tex,3,1), vertex_dtdt); 
    ...
    if original == 0
        cur_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_profile_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal.bmp'];
    else
        cur_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_original_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal.bmp'];
    end
...
%         if abs(P_angle_y{cnt}) > 10 
        if P_angle ~= 0 | original == 0
            ...
            if half == 1
                if P_angle_y{cnt} > 15
                    cur_img = cur_img( :, 1 : (window_width - 1)/2);
                elseif P_angle_y{cnt} < -15
                    cur_img = cur_img( :, (window_width - 1)/2 + 2 : window_width);
                end
            end
         end
    if original == 0   
        if half == 0
            ...
            cur_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_half_profile_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal.bmp'];            
        end
    else
        if half == 0
            cur_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_original_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal.bmp'];
        else
            cur_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_half_original_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal_1.bmp'];
            cur_img1 = cur_img( :, 1 : (window_width - 1)/2);
            imwrite( cur_img1, cur_img_dir);
            cur_img = cur_img( :, (window_width - 1)/2 + 2 : window_width);
            cur_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_half_original_frontal\' out_image_name(1:end-4) '_' num2str(cnt) '_frontal_2.bmp'];
        end
    end
    ...
    if original == 0
        if half == 0
            resize_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_profile_frontal_resize\' out_image_name(1:end-4) '_' num2str(cnt) '_resize.bmp'];
        else
            resize_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_half_profile_frontal_resize\' out_image_name(1:end-4) '_' num2str(cnt) '_resize.bmp'];
        end    
    else
        if half == 0
            resize_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_original_frontal_resize\' out_image_name(1:end-4) '_' num2str(cnt) '_resize.bmp'];
        else
            resize_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_half_original_frontal_resize\' out_image_name(1:end-4) '_' num2str(cnt) '_resize_1.bmp'];
            resize_img1 = imresize(cur_img1,0.2,'bilinear');
            imwrite( resize_img1, resize_img_dir);
            resize_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_half_original_frontal_resize\' out_image_name(1:end-4) '_' num2str(cnt) '_resize_2.bmp'];            
        end
    end
    imwrite(resize_img,resize_img_dir); 
    crop_img = imcrop( resize_img, [26 8 99 119]);    
    crop_img_dir = ['D:\learning_opengl\BaselFace\02_LFW_profile_frontal_resize_crop\' out_image_name(1:end-4) '_' num2str(cnt) '_crop.bmp'];
    imwrite(crop_img,crop_img_dir); 
    tim = toc;
