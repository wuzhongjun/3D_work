...
%  whole_ans_v4 = zeros(3,int32(size(img_dir,1)));
load('whole_ans_v4.mat','whole_ans_v4');
num = 1;
if num > 213
    num = num - 1;
end
...
for i = 30:size(img_dir,1)
%  for i = (a.*7+ii-7)
    if find(a == i)
        fprintf('i=%d\n',i);
        std_num = str2num(img_dir(i).name(1:3));
        [dist_index,P_angle] = face_recognition_m_depth(strcat('D:\learning_opengl\MPIE\croppedimg2\',img_dir(i).name),img_str ,std_num,i);
    %     pause; 

        whole_ans_v4(1,i) = std_num;    
        whole_ans_v4(2,i) = dist_index;
        whole_ans_v4(3,i) = int32(P_angle);
        save('whole_ans_v4.mat','whole_ans_v4');
%         pause;
    else
        fprintf('i=%d\n',i);
        std_num = str2num(img_dir(i).name(1:3));
        whole_ans_v4(1,i) = std_num;    
        whole_ans_v4(2,i) = std_num;
        save('whole_ans_v4.mat','whole_ans_v4');
    end
%     pause;
end
...
rec_rate2 = zeros(1,7);
for i = 1:7
    rec_rate(i) = size(find(whole_ans_v4(1,[i:7:1743])==whole_ans_v4(2,[i:7:1743])),2)/249;
end
